...
export const psiAccounts = pgTable(
  "psi_accounts",
  {
...
export const repCommissions = pgTable(
  ...
  (table) => [
    ...
    check(
      "payout_status_check",
      sql`(payout_status)::text = ANY ((ARRAY['Pending'::character varying, 'Paid'::character varying])::text[])`,
    ),
    check(
...
export const refCodes = pgTable(
  ...
  (table) => [
    index("idx_ref_codes_list_name").using(
      "btree",
      table.listName.asc().nullsLast().op("text_ops"),
    ),
  ],
);

export const listValues = pgTable(
  "list_values",
  {
    id: serial().primaryKey().notNull(),
  ...
  (table) => [
    index("idx_list_values_list_name").using(
      "btree",
      table.listName.asc().nullsLast().op("text_ops"),
    ),
  ],
