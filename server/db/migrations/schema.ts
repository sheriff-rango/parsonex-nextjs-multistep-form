...
export const usersUseridSeq = pgSequence("users_userid_seq", {  startWith: "1", increment: "1", minValue: "1", maxValue: "9223372036854775807", cache: "1", cycle: false })
export const psiHoldingsHoldingIdSeq = pgSequence("psi_holdings_holding_id_seq", {  startWith: "1", increment: "1", minValue: "1", maxValue: "9223372036854775807", cache: "1", cycle: false })
export const repsRepIdSeq = pgSequence("reps_rep_id_seq", {  startWith: "1", increment: "1", minValue: "1", maxValue: "9223372036854775807", cache: "1", cycle: false })
...
export const psiAccounts = pgTable("psi_accounts", {
	accountId: varchar("account_id").default(nextval(\'psi_accounts_psiaccountid_seq\'::regclass)).primaryKey().notNull(),
	...
	clientIdPrimary: varchar("client_id_primary"),
	clientIdJoint: varchar("client_id_joint"),
	status: varchar(),
	...
	foreignKey({
			columns: [table.clientIdPrimary],
			foreignColumns: [clients.clientId],
			name: "fk_client_primary"
		}).onDelete("set null"),
]);

export const wfItemStatusHistory = pgTable("wf_item_status_history", {
	historyid: serial().primaryKey().notNull(),
	workItemId: integer("work_item_id"),
	statusId: integer("status_id"),
	changedBy: varchar("changed_by"),
	changedOn: timestamp("changed_on", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	notes: text(),
}, (table) => [
	foreignKey({
...
export const repRegistrations = pgTable("rep_registrations", {
	...
	index("idx_rep_registrations_repid").using("btree", table.repId.asc().nullsLast().op("int4_ops")),
	index("idx_rep_registrations_type").using("btree", table.registrationType.asc().nullsLast().op("text_ops")),
	check("chk_registration_status", sql`(status)::text = ANY ((ARRAY['Active'::character varying, 'Pending'::character varying, 'Terminated'::character varying])::text[])`),
]);

export const repCommissions = pgTable("rep_commissions", {
	...
	commissionType: varchar("commission_type", { length: 20 }).default('Initial'),
	batchId: integer("batch_id"),
}, (table) => [
	index("idx_rep_commissions_account_id").using("btree", table.accountId.asc().nullsLast().op("int4_ops")),
	index("idx_rep_commissions_rep_id").using("btree", table.repId.asc().nullsLast().op("int4_ops")),
	index("idx_rep_commissions_transaction_id").using("btree", table.orderId.asc().nullsLast().op("int4_ops")),
	check("payout_status_check", sql`(payout_status)::text = ANY ((ARRAY['Pending'::character varying, 'Paid'::character varying])::text[])`),
	check("rep_commissions_calculated_amount_check", sql`calculated_amount >= (0)::numeric`),
]);
...
export const phones = pgTable("phones", {
	...
	check("phones_phone_type_check", sql`(phone_type)::text = ANY ((ARRAY['mobile'::character varying, 'home'::character varying, 'work'::character varying, 'other'::character varying])::text[])`),
]);
...
export const wfItem = pgTable("wf_item", {
	...
	index("idx_wf_item_assignedto").using("btree", table.assignedTo.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.statusId],
			foreignColumns: [wfStatus.statusId],
...
export const wfItemQueue = pgTable("wf_item_queue", {
	...
	index("idx_wf_item_queue_roleid").using("btree", table.roleId.asc().nullsLast().op("int4_ops")),
	index("idx_wf_item_queue_userid").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.workItemId],
			foreignColumns: [wfItem.workItemId],
...
export const bankInfo = pgTable("bank_info", {
	...
	index("idx_bank_info_accountid").using("btree", table.accountId.asc().nullsLast().op("int4_ops")),
	check("bank_info_related_entity_type_check", sql`(related_entity_type)::text = ANY ((ARRAY['Rep'::character varying, 'Contact'::character varying, 'Client'::character varying])::text[])`),
]);
...
export const notifications = pgTable("notifications", {
	...
	index("idx_notifications_userid").using("btree", table.userid.asc().nullsLast().op("int4_ops")),
]);
...
export const summaryProduction = pgTable("summary_production", {
	...
	productType: varchar("product_type", { length: 50 }).notNull(),
	investmentAmt: numeric("investment_amt", { precision: 15, scale:  2 }).notNull(),
...
export const auditLog = pgTable("audit_log", {
	...
	index("idx_audit_log_tablename").using("btree", table.tablename.asc().nullsLast().op("text_ops")),
]);
...
export const productFamily = pgTable("product_family", {
	...
	index("idx_product_family_familyname").using("btree", table.familyName.asc().nullsLast().op("text_ops")),
]);
...
export const wfRoles = pgTable("wf_roles", {
	...
	description: text(),
	wfType: varchar("wf_type"),
});

export const pasHoldings = pgTable("pas_holdings", {
	...
	marketValue: numeric("market_value", { precision: 18, scale:  4 }).generatedAlwaysAs(sql`(units * unit_price)`),
...
export const documents = pgTable("documents", {
	...
	index("idx_documents_relatedid").using("btree", table.relatedid.asc().nullsLast().op("int4_ops")),
	check("chk_documents_relatedtype", sql`(relatedtype)::text = ANY ((ARRAY['account'::character varying, 'rep'::character varying, 'compliance'::character varying, 'client'::character varying])::text[])`),
]);
...
export const repOba = pgTable("rep_oba", {
	...
	createdon: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	lastupdated: timestamp({ mode: 'string' }),
});
...
export const userPermissions = pgTable("user_permissions", {
	...
	index("idx_user_permissions_role").using("btree", table.role.asc().nullsLast().op("text_ops")),
	index("idx_user_permissions_userid_role").using("btree", table.userid.asc().nullsLast().op("int4_ops"), table.role.asc().nullsLast().op("int4_ops")),
]);

export const fanmailDownloads = pgTable("fanmail_downloads", {
	fileId: serial("file_id").primaryKey().notNull(),
	fileName: varchar("file_name", { length: 255 }).notNull(),
	filePath: varchar("file_path", { length: 255 }).notNull(),
	fileType: varchar("file_type", { length: 50 }).notNull(),
	uploadDate: timestamp("upload_date", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	processedDate: timestamp("processed_date", { mode: 'string' }),
	status: varchar({ length: 50 }).default('Pending').notNull(),
	errorMessage: text("error_message"),
}, (table) => [
	index("idx_fanmail_file_name").using("btree", table.fileName.asc().nullsLast().op("text_ops")),
	index("idx_fanmail_upload_date").using("btree", table.uploadDate.asc().nullsLast().op("timestamp_ops")),
	check("chk_fanmail_status", sql`(status)::text = ANY ((ARRAY['Pending'::character varying, 'Processed'::character varying, 'Failed'::character varying])::text[])`),
]);

export const repLicenses = pgTable("rep_licenses", {
	...
	index("idx_rep_licenses_state").using("btree", table.state.asc().nullsLast().op("text_ops")),
]);
...
export const clients = pgTable("clients", {
	clientId: varchar("client_id").default(nextval(\'clients_clientid_seq\'::regclass)).primaryKey().notNull(),
...
export const psiHeldaway = pgTable("psi_heldaway", {
	...
	index("idx_psi_heldaway_financialprofileid").using("btree", table.accountid.asc().nullsLast().op("int4_ops")),
]);

export const pasSis = pgTable("pas_sis", {
	...
	index("idx_pas_sis_accountid").using("btree", table.accountid.asc().nullsLast().op("int4_ops")),
	check("chk_pas_sis_percentage", sql`(percentage >= (0)::numeric) AND (percentage <= (100)::numeric)`),
]);
...
export const complianceChecks = pgTable("compliance_checks", {
	...
	index("idx_compliance_checks_checktype").using("btree", table.checktype.asc().nullsLast().op("text_ops")),
]);

export const repFees = pgTable("rep_fees", {
	...
	check("rep_fees_payment_method_check", sql`(payment_method)::text = ANY ((ARRAY['Deduct from Commission'::character varying, 'Pre-pay'::character varying])::text[])`),
]);
...
export const psiHoldingsTemp = pgTable("psi_holdings_temp", {
	holdingId: varchar("holding_id"),
	accountId: varchar("account_id"),
	holdingType: varchar("holding_type"),
	securityName: varchar("security_name"),
	securityTicker: varchar("security_ticker"),
	cusip: varchar(),
	units: numeric(),
	...
	holdingFan: varchar("holding_fan"),
...
export const rep1099Summary = pgTable("rep_1099_summary", {
	summaryId: serial("summary_id").primaryKey().notNull(),
	repId: varchar("rep_id"),
	taxYear: integer("tax_year").notNull(),
	totalCommission: numeric("total_commission", { precision: 18, scale:  2 }).notNull(),
	...
	check("rep_1099_summary_total_commission_check", sql`total_commission >= (0)::numeric`),
	check("rep_1099_summary_net_income_check", sql`net_income >= (0)::numeric`),
]);

export const households = pgTable("households", {
	householdId: text("household_id").default(nextval(\'pas_households_householdid_seq\'::regclass)).primaryKey().notNull(),
...
export const emailTracking = pgTable("email_tracking", {
	...
	trackingPixelUrl: varchar("tracking_pixel_url", { length: 255 }),
});

export const refCodes = pgTable("ref_codes", {
	id: serial().primaryKey().notNull(),
	...
	index("idx_ref_codes_list_name").using("btree", table.listName.asc().nullsLast().op("text_ops")),
]);
...
export const listValues = pgTable("list_values", {
	...
	index("idx_list_values_list_name").using("btree", table.listName.asc().nullsLast().op("text_ops")),
]);
...
export const psiOrdersTemp = pgTable("psi_orders_temp", {
	...
	dealerComCode: varchar("dealer_com_code"),
	tin: integer(),
	recordCode: integer("record_code"),
});

export const wfStatus = pgTable("wf_status", {
	...
	publicStatus: varchar("public_status"),
}, (table) => [
	foreignKey({
...
export const userActivity = pgTable("user_activity", {
	...
	index("idx_user_activity_login_timestamp").using("btree", table.loginTimestamp.asc().nullsLast().op("timestamp_ops")),
	index("idx_user_activity_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
]);
...
export const instructions = pgTable("instructions", {
	...
	index("idx_instructions_accountid").using("btree", table.accountid.asc().nullsLast().op("int4_ops")),
	index("idx_instructions_contactid").using("btree", table.contactid.asc().nullsLast().op("int4_ops")),
	index("idx_instructions_instructiontype").using("btree", table.instructiontype.asc().nullsLast().op("text_ops")),
]);
...
export const psiSuitability = pgTable("psi_suitability", {
	...
	index("idx_psi_suitability_investmenttype").using("btree", table.investmenttype.asc().nullsLast().op("text_ops")),
]);
...
export const psiHoldings = pgTable("psi_holdings", {
	...
	branchNo: varchar("branch_no"),
	repNo: varchar("rep_no"),
	holdingId: integer("holding_id").default(sql`nextval('psi_holdings_holding_id_seq'::regclass)`).primaryKey().notNull(),
});

export const psiBlotter = pgTable("psi_blotter", {
...
export const reps = pgTable("reps", {
	...
	fullname: varchar(),
	isActive: boolean("is_active").default(true),
	repType: varchar("rep_type", { length: 50 }).default('Unassigned').notNull(),
	...
	crd: varchar({ length: 20 }),
	npn: varchar({ length: 20 }),
}, (table) => [
	index("idx_reps_pcm").using("btree", table.pcm.asc().nullsLast().op("text_ops")),
]);
...
export const notes = pgTable("notes", {
	...
	index("idx_notes_related").using("btree", table.relatedid.asc().nullsLast().op("int4_ops"), table.relatedtype.asc().nullsLast().op("int4_ops")),
]);
...
export const repCommissionAdjustments = pgTable("rep_commission_adjustments", {
	...
	check("rep_commission_adjustments_adjustment_amount_check", sql`adjustment_amount >= (0)::numeric`),
]);

export const psiOrders = pgTable("psi_orders", {
	...
	index("idx_psi_orders_repsearchid").using("btree", table.fanRepName.asc().nullsLast().op("text_ops")),
	index("idx_psi_orders_tradedate").using("btree", table.tradeDate.asc().nullsLast().op("date_ops")),
]);

export const repCommissionStatements = pgTable("rep_commission_statements", {
	...
	check("rep_commission_statements_total_commission_check", sql`total_commission >= (0)::numeric`),
	check("rep_commission_statements_net_payout_check", sql`net_payout >= (0)::numeric`),
]);
